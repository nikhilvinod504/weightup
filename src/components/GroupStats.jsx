...
// ── Compute all stats from raw logs + goal ────────────────────────────────────
...
function Card({ children }) {
  return (
    <div style={{ background:"#fff", borderRadius:16, padding:"1.25rem", marginBottom:12, border:"0.5px solid #f0f0f0" }}>
      {children}
    </div>
  )
}
...
// ── 1. Overlaid trend lines ───────────────────────────────────────────────────
function TrendChart({ members }) {
  const W = 300, H = 130
  const allW = members.flatMap(m => m.last14.map(l => l.w)).filter(Boolean)
  if (!allW.length) return (
    <div style={{ color:"#bbb", fontSize:13, textAlign:"center", padding:"1.5rem 0" }}>
      Not enough data yet — log a few days first!
    </div>
  )
  const mn = Math.min(...allW) - 1, mx = Math.max(...allW) + 1
  const px = (v, i, total) => [
    8 + (i / Math.max(total-1, 1)) * (W-16),
    H - 8 - ((v-mn)/(mx-mn)) * (H-22)
  ]
  return (
    <>
        ...
        {members.map(m => {
          ...
          return (
            <g key={m.username}>
              ...
              <text x={last[0]+6} y={last[1]+4} fontSize="9" fill={m.color} fontWeight="700">
                {m.name.split(" ")[0]}
              </text>
            </g>
          )
    ...
    <div>
      <div style={{ fontSize:12, fontWeight:600, color:P[800], marginBottom:5 }}>
        ...
        <span style={{ fontSize:10, color:"#bbb", fontWeight:400, marginLeft:6 }}>
          {days.filter(d=>d.logged).length}/30 days
        </span>
      </div>
...
function WeeklyBars({ members }) {
  ...
  if (!allVals.length) return (
    <div style={{ color:"#bbb", fontSize:13, textAlign:"center", padding:"1rem" }}>
          ...
          vals.map((v,mi) => {
            ...
            return (
              <g key={`${di}-${mi}`}>
                ...
                {bh > 20 && <text x={x+(barW-1)/2} y={H-16-bh-3} fontSize="7" fill={members[mi].color} textAnchor="middle">{v}</text>}
              </g>
            )
          })
        )}
...
export default function GroupStats({ group }) {
  ...
  if (loading) return (
    <div style={{ textAlign:"center", padding:"3rem 0" }}>
      ...
      <div style={{ fontSize:13, color:"#bbb" }}>Loading group stats…</div>
    </div>
  )

  if (error) return (
    <div style={{ textAlign:"center", padding:"2rem", color:C[400], fontSize:13 }}>
      {error}
    </div>
  )
  ...
  const hasAnyLogs = data.some(m => m.logs.length > 0)
  if (!hasAnyLogs) return (
    <div style={{ textAlign:"center", padding:"3rem 1rem" }}>
      <div style={{ fontSize:40, marginBottom:12 }}>🌱</div>
      <div style={{ fontSize:15, fontWeight:600, color:P[800], marginBottom:6 }}>No data yet</div>
      <div style={{ fontSize:13, color:"#aaa" }}>Once members start logging weights, the charts will appear here.</div>
    </div>
  )
  ...
  return (
    <div>
      ...
      <Card>
        <SectionHeader icon="⬇️" title="Total kg lost" subtitle="From starting weight to today"/>
      ...
      <Card>
        ...
        <HBar members={data} getValue={m=>m.weekChange} fmt={v=>v>0?`▼ ${v} kg`:v<0?`▲ ${Math.abs(v)} kg`:"no change"} colorBySign/>
      </Card>
