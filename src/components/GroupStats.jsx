...
import { stGet } from '../utils/storage.js'
...
function getMemberData(members) {
  return members.map((m, i) => {
    const logs   = stGet(`logs:${m.username}`) || []
    const goalD  = stGet(`goal:${m.username}`)
    ...
    return { ...m, color: MEMBER_COLORS[i % MEMBER_COLORS.length], logs, goal, streak, xp, level, first, latest, lost, goalPct, weekChange, last14, logDates, badgeCount: badges.length }
  })
}
    ...
    <div style={{ display:"flex", gap:10, flexWrap:"wrap", marginTop:8 }}>
      {members.map(m => (
        <div key={m.username} style={{ display:"flex", alignItems:"center", gap:4 }}>
          <div style={{ width:10, height:4, borderRadius:2, background:m.color }}/>
...
function TrendChart({ members }) {
  ...
  if (!allW.length) return <div style={{ color:"#bbb", fontSize:13, textAlign:"center", padding:"1rem" }}>Not enough data yet</div>
  const mn = Math.min(...allW) - 1, mx = Math.max(...allW) + 1
  const px = (v, i, total) => [8 + (i / Math.max(total-1,1)) * (W-16), H-8 - ((v-mn)/(mx-mn)) * (H-20)]
        ...
        <div key={m.username}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4 }}>
            <div style={{ display:"flex", alignItems:"center", gap:6 }}>
              ...
              {!m.goal && <span style={{ fontSize:10, color:"#ccc" }}>no goal</span>}
            </div>
          ...
          <div style={{ position:"relative", height:22, background:"#f0f0f0", borderRadius:11 }}>
            ...
            <div style={{ position:"absolute", right:8, top:"50%", transform:"translateY(-50%)", fontSize:11 }}>🏁</div>
            {m.goalPct > 8 && (
              <div style={{ position:"absolute", left:`${Math.max(m.goalPct,2)}%`, top:-10, transform:"translateX(-50%)", fontSize:14 }}>🏃</div>
            )}
          </div>
        </div>
      ))}
...
function HeatmapRow({ member }) {
  ...
  return (
    <div>
      <div style={{ fontSize:12, fontWeight:600, color:P[800], marginBottom:5 }}>{member.name.split(" ")[0]}</div>
...
function WeeklyBars({ members }) {
  ...
  if (!allVals.length) return <div style={{ color:"#bbb", fontSize:13, textAlign:"center", padding:"1rem" }}>No data this week yet</div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width:"100%", height:H }}>
        {dayData.map((vals,di) =>
          vals.map((v,mi) => {
            ...
            return <rect key={`${di}-${mi}`} x={x} y={H-14-bh} width={barW-1} height={bh} fill={members[mi].color} rx="2" opacity="0.85"/>
          })
        )}
        {dayLabels.map((d,i) => (
          <text key={d} x={8+i*barGroupW+barGroupW/2} y={H-2} fontSize="8" fill="#bbb" textAnchor="middle">{d}</text>
        ))}
      </svg>
      <Legend members={members}/>
    </>
...
function FunAwards({ members }) {
  ...
  const awards = [
    { icon:"📅", title:"Most logs",     winner:top((a,b)=>b.logs.length-a.logs.length),  stat:m=>`${m.logs.length} total logs`,    color:T[400] },
    { icon:"📉", title:"Biggest drop",  winner:top((a,b)=>b.lost-a.lost),                stat:m=>`${m.lost}kg lost`,               color:P[400] },
...
export default function GroupStats({ group }) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    try { setData(getMemberData(group.members)) }
    catch(e) { console.error(e) }
    finally { setLoading(false) }
  }, [group?.id])

  if (loading) return <div style={{ textAlign:"center", color:"#bbb", fontSize:13, padding:"2rem" }}>Loading stats…</div>
  if (!data.length) return <div style={{ textAlign:"center", color:"#bbb", fontSize:13, padding:"2rem" }}>No members yet</div>

  return (
    <div>
      <Card>
        <SectionHeader icon="📈" title="Weight trends" subtitle="Everyone's last 14 days overlaid"/>
        <TrendChart members={data}/>
      </Card>

      <Card>
        <SectionHeader icon="⬇️" title="Total kg lost" subtitle="From start weight to today"/>
        <HBar members={data} getValue={m=>Math.max(m.lost,0)} fmt={v=>`${v} kg`}/>
      </Card>

      <Card>
        <SectionHeader icon="🏁" title="Goal race" subtitle="% of goal reached — who's winning?"/>
        <RaceTrack members={data}/>
      </Card>

      <Card>
        <SectionHeader icon="📆" title="This week's change" subtitle="kg lost in the last 7 days"/>
        <HBar members={data} getValue={m=>m.weekChange} fmt={v=>v>0?`▼ ${v} kg`:v<0?`▲ ${Math.abs(v)} kg`:"—"} colorBySign/>
      </Card>

      <Card>
        <SectionHeader icon="📊" title="Daily weights this week" subtitle="Each bar = a day, colours = members"/>
        <WeeklyBars members={data}/>
      </Card>

      <Card>
        <SectionHeader icon="🔥" title="Logging streaks" subtitle="Consecutive days logged"/>
        <HBar members={data} getValue={m=>m.streak} fmt={v=>`${v} days`}/>
      </Card>

      <Card>
        <SectionHeader icon="🗓️" title="Consistency heatmap" subtitle="Last 30 days — coloured = logged"/>
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          {data.map(m => <HeatmapRow key={m.username} member={m}/>)}
        </div>
        <div style={{ display:"flex", gap:10, marginTop:10, fontSize:10, color:"#bbb", alignItems:"center" }}>
          <div style={{ width:11, height:11, borderRadius:2, background:"#efefef" }}/> missed
          <div style={{ width:11, height:11, borderRadius:2, background:P[400] }}/> logged
        </div>
      </Card>

      <Card>
        <SectionHeader icon="⚡" title="XP comparison" subtitle="Total experience points earned"/>
        <HBar members={data} getValue={m=>m.xp} fmt={v=>`${v} XP`}/>
      </Card>

      <Card>
        <SectionHeader icon="🪜" title="Level ladder" subtitle="Where everyone stands"/>
        <LevelLadder members={data}/>
      </Card>

      <Card>
        <SectionHeader icon="🏆" title="Group awards" subtitle="Standouts across all categories"/>
        <FunAwards members={data}/>
      </Card>
    </div>
  )
}
